        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 bg-white/[0.02] backdrop-blur-xl border border-white/5 p-6 sm:p-8 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl">
            <div className="space-y-6">
              ...
              <button disabled={!password || !title || isLoading} onClick={saveToDatabase} className="w-full bg-blue-600 hover:bg-blue-500 p-4 sm:p-5 rounded-2xl font-black text-white text-[10px] tracking-widest shadow-xl uppercase active:scale-95 transition-transform">
                {isLoading ? 'Procesando...' : 'Guardar en Bóveda'}
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="bg-white/5 p-4 rounded-2xl border border-white/5 flex items-center gap-4">
              ...
              <input type="text" placeholder="Buscar credencial..." className="bg-transparent border-none outline-none text-sm text-white w-full" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>

            <div className="space-y-4 max-h-[500px] sm:max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {filteredPasswords.map((item) => (
                <div key={item.id} className="group bg-white/[0.01] border border-white/5 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] flex justify-between items-center hover:bg-white/[0.03] transition-all">
              ...
              {filteredPasswords.length === 0 && (
                <div className="text-center py-12 border border-dashed border-white/5 rounded-3xl">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-600">No se encontraron registros</p>
                </div>
              )}
            </div>
          </div>
